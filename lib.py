...
def execute_on_BlueQbit(qc, num_shots=500, show_results=None, num_s_bits=2, job_id="", conf={}):
    ...
    BLUEQUBIT_TOKEN = os.environ.get("BLUEQUBIT_TOKEN")
    if not BLUEQUBIT_TOKEN:
        raise ValueError(
            "BLUEQUBIT_TOKEN environment variable is not set. "
            "Please set it in your .env file or environment variables."
        )
    
    bq_client = bluequbit.init(BLUEQUBIT_TOKEN)
...
def simulate(qc, num_shots=300):
    from qiskit_aer import AerSimulator
    # Use AerSimulator with matrix_product_state (MPS) method for large circuits
    # This avoids storing the full quantum state (2^N for N qubits) and is memory-efficient
    # For 53 qubits, statevector would need 2^53 * 16 bytes ≈ 17TB, which is impossible
    # MPS method uses tensor network representation, much more memory-efficient for large circuits
    # Alternative: use method='automatic' to let AerSimulator choose the best method
    backend = AerSimulator(method='matrix_product_state')
    # Transpile to decompose multi-controlled gates (like c4z) into basic gates
    # Use basis_gates instead of backend to avoid coupling map constraints
    # This only decomposes gates without checking qubit limits
    # Include standard gates: h, x, z, cx, and others that AerSimulator supports
    basis_gates = ['id', 'rz', 'sx', 'x', 'y', 'z', 'h', 'cx', 'cy', 'cz', 'reset', 'measure']
